...
description: >
  Extract SAME field from MANY config files. TRIGGERS: 'all pom.xml', 'check all modules', 'audit all configurations',
  'list all versions', 'extract from every file', 'scan all catalogs', 'find in all components', 'get field from multiple',
  'report across all', 'batch extract', 'all configs', 'check every file', 'list all dependencies', 'find in all yamls',
  'scan every pom', 'generate report', 'get from all files', 'extract all versions', 'list all packages', 'audit all
  systems', 'find all owners', 'check all properties', 'count all items', 'find in all files', 'export all metadata',
  'verify all components', 'check all annotations'. Use when same field needed from 10+ files (not one-by-one reading).
  Essential for: Maven multi-module audits (versions, scopes from 10+ poms), Backstage catalogs (metadata, owners from
  100+ entities), batch config validation (properties across many files), compliance reports, metadata exports,
  dashboards from distributed config, security checks across repos.
---
